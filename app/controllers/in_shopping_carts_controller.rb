class InShoppingCartsController < ApplicationController
	def create
		...
		in_shopping_cart = InShoppingCart.new(product_id: params[:product_id],in_shopping_cart: @shopping_cart)

		if in_shopping_cart.save
			redirect_to carrito_path, notice: "Guardamos el producto en tu carrito"
		else
			redirect_to products_path(id: params[:product_id]), notice: "No pudimos agrgar al carrito, intenta de nuevo"
		end
	end
